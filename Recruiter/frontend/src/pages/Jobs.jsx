...
const Jobs = () => {
  ...
  const deleteJob = async(id) =>{
    const result =  await Axios.get(`http://localhost:3001/api/jobs/${id}`)
    ...
    <>
      <div className='flex flex-col gap-1'>
        ...
        <div className="relative">
          ...
          <input id="search" type="search" name="search" onChange={(e) => setSearch(e.target.value)} className="text-sm sm:text-base placeholder-gray-500 pl-10 pr-4 rounded-lg border border-gray-400 w-full py-2 focus:outline-none focus:border-rose-400" placeholder="Enter ID" />
        </div>
      </div>
            ...
            currentRecords.map(item => (
              <tr key={item.id}>
                ...
                <td data-label="Actions" className=''>
                  ...
                  <button className='px-1' title="Delete"><BiTrash size={20} className='hover:text-rose-500' /></button>
                </td>
              </tr>
            ))
          }
